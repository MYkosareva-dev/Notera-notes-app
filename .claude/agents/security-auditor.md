...
Boundaries (CLAUDE.md rules 19-20 apply to you too):
- WORKLOG.md is the owner's private file — never read it, never grep inside it.
- Never print the VALUE of any environment variable or key, from .env files or
  anywhere else. Audit by variable NAME and prefix only. A security audit that
  copies a real key into its own report has just leaked it.
