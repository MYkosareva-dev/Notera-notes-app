...
25. A privileged key accidentally pasted into `.env.local` → **refused at boot**, and the scan finds no spelling of it in the repo. Two changes at the security audit (W2, W3). The reviewer scan is `npm run check`, whose needle now covers `service_role\|sb_secret_\|SECRET_KEY\|SERVICE_KEY` case-insensitively, because the old single spelling was one key generation behind this project's own key. And "the app never reads it" is no longer the whole answer: `.env.local` holds `NEXT_PUBLIC_*` variables, which Next inlines TEXTUALLY into every browser bundle, so a secret key there is read by every visitor whether or not app code names it — an RLS bypass published to anyone who views source, while the app boots and works perfectly. `lib/supabase/env.ts` therefore throws on any value with the secret-key prefix, before the app serves a byte. Scope stated honestly in that file: it compares a prefix, so it catches the current key format and NOT a legacy `service_role` JWT, which is distinguishable from the legacy `anon` key only by base64-decoding its `role` claim.
...
5. Both greps run over **every code file this repo ships** — `app/`, `components/`, `lib/`, `scripts/`, and the root-level code files (`proxy.ts`, `next.config.ts`, `postcss.config.mjs`, and the committed `.env.example`), plus `supabase/` for the SQL — and return nothing. `.ts`, `.tsx`, `.js`, `.jsx`, `.mjs` and `.cjs` all count: Next's default `pageExtensions` routes `.jsx`, and `tsconfig.json` sets `allowJs: false`, so a `.jsx` page would ship while escaping both this check and `npm run typecheck` — a Phase 7 mutation test proved it. Next's generated `next-env.d.ts` is skipped. Never a `.env*` glob: printing a finding out of a real secrets file would leak the secret, the same reasoning that keeps `WORKLOG.md` out.
   `grep -ril "localStorage\|sessionStorage" app/ components/ lib/ scripts/ proxy.ts next.config.ts postcss.config.mjs .env.example`, and the same over `"service_role\|sb_secret_\|SECRET_KEY\|SERVICE_KEY"` (case-insensitive) with `supabase/` added.
   **The second needle covers four spellings, widened at the security audit on `lab/agents` (finding W2), and the three new ones are what make it mean anything.** `service_role` alone was one key GENERATION behind the key this project actually uses: this project's key is `sb_publishable_`-prefixed, so its paired secret is `sb_secret_…` under a variable conventionally named `SUPABASE_SECRET_KEY` — a name sharing no substring with `service_role`. A new-format secret key pasted into `.env.example`, the most plausible place for it, passed every gate green; `SERVICE_KEY` is in for the same failure one generation earlier, a hand-shortened legacy name. Verified by mutation: all four spellings now FAIL the check, in `.env.example` and in app code alike. Two consequences worth knowing. First, `SECRET_KEY` matches an IDENTIFIER too, so the boot-time guard in `lib/supabase/env.ts` builds its prefix from fragments and is named `FORBIDDEN_PREFIX` rather than after the thing it detects — the same trick `scripts/check.mjs` has always used on itself, for the same reason. Second, prose in app code must now say "the legacy service-role name" and never the underscored form; `lib/theme.ts` already sets that precedent by writing "web storage" rather than naming the API. Root-level code files are listed by name rather than globbed, because the generated `next-env.d.ts` must stay out; `npm run check` is the executable form of record and derives the list itself.
